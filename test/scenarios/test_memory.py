...
async def test_memory_routing_vs_round_robin(
    ...
    print(f"\n[{SCENARIO}] pressured worker = {target_id}")
    print(f"  round_robin share to pressured worker: {rr_share:.1%}")
    print(f"  memory=1    share to pressured worker: {sig_share:.1%}")
    print("  (memory-aware routing should send LESS to the pressured worker)")
